...
public:
    ...
    void update();
    void updateCase(Case c);              // 시나리오 변경
    void forceBreakMachine(int index);    // 강제 고장

    // ── 상태 조회 (UI 데이터 제공) ──────────────────────────
    bool isRunning() const;
    int getTick() const;
    int getTotalBreakdownCount() const;
    int getLostProductCount() const;
    std::vector<std::string> popEventLogs();
...
private:
    ...
    std::vector<bool> power_;

    // 시뮬레이션 상태 및 통계
    int tick_ = 0;
    int totalBreakdownCount_ = 0;
    int lostProductCount_ = 0;
